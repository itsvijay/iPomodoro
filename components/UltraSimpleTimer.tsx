...
export function PomodoroTimer() {
  ...
  // Timer refs - no React state for timer logic
  const timeLeftRef = useRef(settings.focusTime)
  ...
  // Ultra-simple timer using refs only
  const startTimer = () => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        setTimeout(() => {
          ...
          setDisplayTime(formatTime(nextTime))
  ...
  const resetTimer = () => {
    stopTimer()
    timeLeftRef.current = settings.focusTime
    setDisplayTime(formatTime(settings.focusTime))
    setTaskDescription('')
    setSelectedTags([])
    setCurrentTaskState(null)
  }
